...
- **Technology**: Google Gemini 2.0 Flash API via Vercel AI SDK
...
- **How it works**:
  1. Gets user's location via browser geolocation API
  2. Fetches list of all mandis from data.gov.in
  3. Uses Google Maps Distance Matrix API to calculate road distances
  4. Returns prices from the nearest mandi
...
1. **Realistic Network Delay**: 6-10 seconds (matches real ONDC latency)
2. **Buyer Pool**: 8 major verified buyers (Reliance Fresh, BigBasket, Flipkart Grocery, etc.)
3. **Auto-Learning Bids**: Bids learn from historical patterns to become more realistic
4. **Price Calculation**: 95-105% of catalog price (realistic negotiation range)
5. **Full Logging**: All simulated bids logged to database for tracking
...
| AI Processing | ✅ Real | Gemini 2.0 Flash | Yes |
...
| ONDC Network | 🔄 Simulated | Custom Simulator | Yes (bids) |
